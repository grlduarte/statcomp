...
int Ising2D::spin_exchange(int i, int j){
    if (lattice[i] != lattice[j]) { //soh atua se forem diferentes
        int* i_0 = lattice.data();
        int u = up[i] - i_0;
        int d = down[i] - i_0;
        int l = left[i] - i_0;
        int r = right[i] - i_0;

        float d_energy1, d_energy2, d_energy_tot;
        if ((u!=j) && (d!=j) && (l!=j) && (r!=j)) { //verifica se i e j sao vizinhos
            d_energy1 = lattice[i] * (*up[i] + *down[i] + *left[i] + *right[i]);
            d_energy2 = lattice[j] * (*up[j] + *down[j] + *left[j] + *right[j]);
        }
        else { //se forem vizinhos
            d_energy1 = lattice[i] * (*up[i] + *down[i] + *left[i] + *right[i] - lattice[j]);
            d_energy2 = lattice[j] * (*up[j] + *down[j] + *left[j] + *right[j] - lattice[i]);
        }
        d_energy_tot = d_energy1 + d_energy2;
        if ((d_energy_tot <= 0.) || (ranf() < exp(-2.*d_energy_tot/T))) {
            lattice[i] *= -1;
            lattice[j] *= -1;
            return 1;
        }
